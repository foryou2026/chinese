...
function initialsOf(s: string): string {
  ...
  if (!t) return '?';
  // 中文/CJK 取首字符
  if (/[\u4e00-\u9fa5]/.test(t.charAt(0))) return t.slice(0, 1);
  // 邮箱：取 @ 前的首两位
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  const user = session.data?.authenticated === true ? session.data.user : null;
      ...
      <TopNav
        left={
          <>
            ...
            <Link to="/learn" data-testid="nav-learn">{t('nav.courses')}</Link>
          </>
        }
        right={
          <>
