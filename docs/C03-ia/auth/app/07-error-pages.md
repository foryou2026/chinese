<!-- TARGET-PATH: docs/C03-ia/auth/app/07-error-pages.md -->
...
| 403 已登录但访问 admin 入口 | 复用全局 `/403` 毛玻璃页（B03 §02）| api-app 中间件返 `AUTH_FORBIDDEN` |
| 404 路径不存在 | 复用全局 `/404` 毛玻璃页（B03 §02）| router fallback |
