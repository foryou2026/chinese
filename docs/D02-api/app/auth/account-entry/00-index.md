...
> **接口详情**：01-register.md ~ 04-session.md
...
| API-app-auth-forgot-password | POST | /api/v1/app/auth/forgot-password | 发送重置验证码 | 公开 | R-auth-005 | 无 | 03 |
| API-app-auth-verify-reset-otp | POST | /api/v1/app/auth/forgot-password/verify | 验证重置OTP | 公开 | R-auth-005 | 无 | 03 |
| API-app-auth-reset-password | POST | /api/v1/app/auth/reset-password | 重置密码 | 公开(需token) | R-auth-006,010 | 无 | 03 |
| API-app-auth-change-password | PUT | /api/v1/app/auth/password | 修改/设置密码 | Bearer JWT | R-auth-007,010,014 | 无 | 03 |
| API-app-auth-logout | POST | /api/v1/app/auth/logout | 退出登录 | Bearer JWT | R-auth-008 | TR-010 | 04 |
...
| R-ID | 承接 API-ID |
|------|------------|
| R-auth-001 邮箱注册 | register + verify-register |
| R-auth-002 邮箱登录 | login |
| R-auth-003 Google登录 | google-sync (OAuth由前端supabase-js驱动) |
| R-auth-004 登录限流 | login (BR-002/BR-002a) |
| R-auth-005 忘记密码 | forgot-password + verify-reset-otp |
| R-auth-006 重置密码 | reset-password |
| R-auth-007 修改密码 | change-password |
| R-auth-008 退出登录 | logout |
| R-auth-009 Token过期 | 前端supabase-js自动刷新(B01-09) |
| R-auth-010 密码强度 | register, reset-password, change-password |
| R-auth-011 邮箱格式 | 各接口 email 字段校验 |
| R-auth-012 Google自动注册 | google-sync |
| R-auth-013 登录注册切换 | 前端路由 |
| R-auth-014 Google密码管理 | change-password + me |
| R-auth-015 注册验证码 | register + verify-register |
| R-auth-016 多设备限制 | login + google-sync |
...
| SM 转移 | 承接 |
|---------|------|
| TR-001~003 登录流程 | login |
| TR-004 注册 | register + verify-register |
| TR-005~006 Google | google-sync |
| TR-007 锁定 | login (BR-002) |
...
**边界**
- [x] 未写 UI/HTML、未修改 SM、API-ID 带 app 前缀、不跨系统

---

## 99. 待确认问题

无（已全部确认：google-sync 返回 is_new_user；验证码冷却后端 Redis + 前端双重控制；reset_token 使用 Supabase verifyOtp 返回的临时 access_token）
