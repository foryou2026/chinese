...
`course` admin 端是 **内容运营 + 内容点检 + 发布管控** 三位一体的后台，仅服务 `super_admin`（遵从 [docs/C02-permissions/01-roles.md](../../../C02-permissions/01-roles.md) 的 2 角色硬约束）。**不是** 学员侧产品的镜像，而是「线下批量生成 → 导入 → 人工点检 → 上架」流水线的可视化与控制面。
