...
- **问题**:新增错误码 `AUTH_USE_USER_ENTRY` (403,管理端登录检测到非 admin 角色) 需补入 [`02-authz-mechanism.md §4`](./02-authz-mechanism.md) 全局错误码清单;D02 路由阶段建立时再同步落到 `D02-api/auth/admin/04-error-codes.md`(目前 D 阶段尚未启动)
