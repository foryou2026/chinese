...
- 全系统**仅 2 个角色**：`admin`（管理端）+ `user`（应用端）。
- 角色字段位置：JWT 账号元数据.role`；DB 用户账号->>'role'`，trigger 同步到 用户档案` 仅作冗余索引。
...
|--------------------|-------|-------------|------|
| `admin` | 超级管理员 | `admin`（`/admin/*`）| 默认 1 个（`.env` seed）；技术上允许多个；UI 无创建入口；不能删/禁用自己；不出现在任何"用户管理"列表 |
| `user` | 用户 | `app`（`/`）| 所有邮箱 / Google 注册的新账号默认 `role='user'` |

> 当前**严禁**引入第三种角色。运营 / 审核 / 客服 / 内容编辑 / AI 训练师 等诉求一律视为 admin 兼任。
...
### 2.2 管理端（surface=`admin`，角色 `admin` 可见）

| 菜单 | 路由 | feature | 备注 |
|------|------|---------|------|
| 用户管理 | `/admin/users` | `user-management`（未来）| 只列 `role='user'`，**绝不**列任何 admin |
| 发现中国 | `/admin/discover` | `discover-china` | 内容审核 / 上下架 |
| 系统课程 | `/admin/courses` | `course` | 课程管理 / 上下架 |
| 小说专区 | `/admin/novels` | `novels`（未来）| 章节管理 / 审核 |

**管理端当前不暴露**：游戏配置、经济、邀请、客服、安全、内容工厂、i18n 校对、支付配置。

---

## 3. 用户管理菜单的  硬性规则

管理端 `/admin/users` 与对应接口 管理端接口：

1. **只能列出 `role='user'`** 的账号；
2. **绝不**列出任何 `role='admin'`（包括管理员自己）；
3. 列表中"封禁 / 解禁 / 重置密码"操作**只对 `role='user'` 生效**；
4. 后端 SQL **必须**强约束 `WHERE role = 'user'`；
5. 前端按钮可见性仅作兜底，**不能反过来靠前端过滤**。

**反例（禁止）**：返回全表给前端，让前端 `.filter(role !== 'admin')`。一旦前端漏过滤就是越权展示。
...
2. **是否 admin**：`req.user.role === 'admin'` → 中间件 `adminRequired`。

对"自己资源"的所有权判定（如自己的订单 / 自己的学习进度），按 `row.user_id === req.user.id` 直接判断，**不进入角色系统**。当前实现不读取任何"数字权限级别"，统一字符串比较。

---

## 5. 与 feature 的对应关系

| feature ID | 应用端（`user`）| 管理端（`admin`）|
|-----------|--------------|---------------------|
...
| `referral`（未来）| ✅ | ❌ |
| `customer-service`（未来）| ✅ 工单 | ❌（当前无 UI，邮件兜底）|
| `i18n`（未来）| 隐式 | 隐式 |
