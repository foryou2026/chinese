...
> F 层（B01~B03）变更必须列出受影响的所有已开工 feature，并触发它们的 D03 V 重跑。

---

## 2026-05-16 · 批次 23 · 全局规范对齐（Round 2 一致性审计）

- 4 轮机械化脚本 + 8 项手工修订，共修改 **164 个文件**，新建 2 个文件。
...
## 2026-05-20 · 批次 22 · 框架重构 · 权限从 B 阶段下沉到 C 阶段（per feature 增量）

> **触发原因**：用户指出"角色权限是随着功能变化的"——一次性全局权限基线无法承载 feature 增量；故将原 B02 权限阶段下沉为 C02-P，每个 feature 走澄清 → 合并入全局权限目录。F 层从 4 阶段收窄到 3 阶段，C 层从 5 阶段扩到 6 阶段。

### prompt/ 结构（最终版）
- **A-framework**（5 文件不变）：A00-00 README、A00-01 框架总览、A00-02 端到端工作流、A00-03 通用约定、A00-04 文档目录规划。
- **B-foundation** 3 阶段 × 3 文件 = 9：B01-A 架构、B02-X 体验、B03-S 设计。
- **C-product** 6 阶段 × 3 文件 = 18：C01-R 需求、C02-P 角色权限、C03-I IA、C04-N 页面、C05-H 原型、C06-E PRD。
- **D-develop** 3 阶段 × 3 文件 = 9：D01-D 数据、D02-L 接口、D03-V 校验。

### docs/ 目录重命名
| 旧目录 | 新目录 |
|------|------|
| `B02-permissions/` | `C02-permissions/` |
| `B03-ux/` | `B02-ux/` |
| `B04-design/` | `B03-design/` |
| `C02-ia/` | `C03-ia/` |
| `C03-pages/` | `C04-pages/` |
| `C04-prototype/` | `C05-prototype/` |
| `C05-prd/` | `C06-prd/` |

### 文件迁移
- `prompt/B-foundation/B02-P0*` → `prompt/C-product/C02-P0*`（跨目录搬迁）
- 其余 prompt 文件按新编号就地重命名（B03-X→B02-X、B04-S→B03-S、C02-I→C03-I、C03-N→C04-N、C04-H→C05-H、C05-E→C06-E）

### A-framework 重写
- A00-01 框架总览：全部表格按 3-B / 9-C 重构，新增「为何 P 下沉到 C」段落。
- A00-02 端到端工作流：mermaid 改为 3 个 B → for-each-feature 9 个 C 的拓扑；上下游契约表、闸口清单、上下文表 12 阶段全部更新。
- A00-03 通用约定：骨架阶段码列表、多端 surface 章节、严禁项 #9 同步更新。
- A00-04 文档目录规划：14 处补丁，C02-permissions 标注为「全局合并不开 feature 子目录」；新增 C05-prototype 三级 index.html 强约束。

### C02-P 模板正向化
- `C02-P01-用户输入-角色描述.md`：删除"本阶段做什么、不做什么"否定栏与触发提示词中的"不许……"措辞；改为正向 §「本阶段要交什么」+ §「本阶段不涵盖什么（均由其他位置承担）」。
- 上游依赖更新为 C01 R 基线 + B01 `08-surfaces.md` / `09-auth-infra.md` + 已冻结的 C02 全局；落盘路径改为 per-feature `_input/<feature-id>/roles-input.md`。

### C05-H03 原型规范追加
- 新增「三级引导页（index.html）统一规范【强制】」章节：明确 L1 全局 / L2 feature / L3 surface 三级层级，强制使用 `B03-design/prototype-style/{tokens,themes,app}.{css,js}` + `topnav glass-nav` + `data-action="toggle-theme"` 主题切换 + L1/L2 `.glass-card` 网格 + L3 统一表格；禁止内联硬编码、禁止 `index.css/landing.css`。

### docs/C02-permissions 索引重写
- 00-index.md 标题从「B02 · 权限与认证规范」改为「C02 · 角色与权限（全局索引）」；阶段标记改为 `C02-P per feature 增量合并`；文件清单去除已不存在的 `02-auth-flow.md` 和 `05-auth-feature-guideline.md`（登录流细节已下放到 `auth` feature 的 C04/C05/C06）；跨文件约定章节同步引用新文件名。

### 验证
- `grep -rE 'B02-permissions|B03-ux|B04-design|C02-ia|C03-pages|C04-prototype|C05-prd|B02-P0|B03-X0|B04-S0|C02-I0|C03-N0|C04-H0|C05-E0' docs/ prompt/` → 0 命中（仅 changelog 历史与 `_input/` 用户草稿保留作历史快照）。
- 所有 `docs/C05-prototype/**/*.html` 三件套引用均走 `../../../B03-design/prototype-style/`，浏览器直接打开 OK。
...
### 死链 / 死引用清除
- **`grules/Gx` 链接（22 处）**：B03-design/design-system/ 19 个文件 + 3 个 C 层 PRD 文件中所有指向 `grules/G2-视觉与交互风格/` / `grules/G3-权限与认证规范/` 的链接全部改写：
  - `grules/G3-权限与认证规范/01-角色定义.md` → [`docs/C02-permissions/01-roles.md`](../C02-permissions/01-roles.md)
  - `grules/G3-.../02-认证流程.md` → [`docs/C02-permissions/02-auth-flow.md`](../C02-permissions/02-auth-flow.md)
  - `grules/G2-.../04-状态与组件.md` → [`docs/B03-design/design-system/04-status-colors.md`](../B03-design/design-system/04-status-colors.md)
...
- **C 层 _input 文件**（11 处）：所有"反向回写 / 反推确认 by PM / 真实背景：C02-permissions 已成型时本 feature 才反向重打包 / 反向回写期补登 / 反向回写期直接采纳"措辞全部正向化或删除——文件读起来像首次按 /prompt 走 C01-R01..C05-H01 用户输入跑出。涉及 auth、course、discover-china 三个 feature 的 C01/C02/C03/C04/C05 _input。
...
> **触发原因**：用户准备删除 /backup（含 /system /function /grules /env.md）；review 时发现 /docs 与 /prompt 多处仍写有"feature 若在 /function/<feature>/ai/F4-AI-原型设计/ 有上游 AI 原型，则改为引用上游 _assets/"这类例外条款，且 [docs/C05-prototype/course/](../C05-prototype/course/) 20 个 HTML 实际硬依赖 `function/02-course/ai/F4-AI-原型设计/_assets/`。用户原话："哪有什么上游文件啊！所有的都是统一的！以后所有都是 0-1 生成，之前的反推是因为软件已经做了一部分了是特殊情况！"——明令删除全部"F4 上游"措辞、迁移 course 资产至 docs/B04、把所有反推阶段对 `/function/` 的引用统一中性化为〔历史素材〕。

### 资产迁移
- 复制 `backup/function/02-course/ai/F4-AI-原型设计/_assets/{styles.css,prototype.js}` → [`docs/B03-design/prototype-style/course/{styles.css,prototype.js}`](../B03-design/prototype-style/course/)，自此 course 专属视觉样式（zy-* 类名 / 节学习页 / KP 列表等）的唯一仓位在 docs/B04 内。
- 批量改写 19 个 course HTML（[`docs/C05-prototype/course/app/`](../C05-prototype/course/app/) 9 + [`docs/C05-prototype/course/admin/`](../C05-prototype/course/admin/) 10）：`../../../../function/02-course/ai/F4-AI-原型设计/_assets/X` → `../../../B03-design/prototype-style/course/X`（4 ups → 3 ups），同时把 `<title>` 中的 "F4 原型" 改为 "C04 原型"。
- [docs/C05-prototype/index.html](../C05-prototype/index.html) 删除"course 引用自 function/02-course/ai/F4-AI-原型设计/"等描述与"X 页 / N 状态"骨架统计，统一为"N 页面默认态"。

### 规范条款清剿（doctrine 删除）
- [docs/B03-design/prototype-style/README.md](../B03-design/prototype-style/README.md) §2.2 由"例外（F4 上游）"重写为"feature 专属样式包（按需）"——允许在 `prototype-style/<feature>/` 下放置 feature 专属 `styles.css` / `prototype.js`，仍 3 ups、只引用不拷贝；**禁止**引用 `/function/` 下任何路径。
...
- [prompt/B-foundation/B03-S03-AI输出-设计系统.md](../../prompt/B-foundation/B03-S03-AI输出-设计系统.md) §引入方式 删除"例外：F4 上游资产"句。
- [prompt/C-product/C05-H01-用户输入-原型方向.md](../../prompt/C-product/C05-H01-用户输入-原型方向.md) 删除"参照范例：function/02-course/ai/F4-AI-原型设计/"、"是否引用上游 F4 资产"复选项、验收清单"或 F4 上游 _assets/"等。
- [prompt/C-product/C05-H03-AI输出-HTML原型规范.md](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md) 四处："参照范例 F4..."、"例外（上游 AI 原型来源）..."、§硬约束 0 子项"例外..."、§<page-id>.html "例外..."、§自检"或 feature 有 F4 上游时..."——全部改为"允许 feature 专属样式包放 docs/B04/prototype-style/<feature>/"或直接删除。
...
- 后续所有 0→1 新建 feature 的 C04，必须只引用 `docs/B03-design/prototype-style/` 内的通用包或 `<feature>/` 子目录的专属包；**禁止**任何 `/function/` 字样的相对路径写入 HTML。

---



> **触发原因**：用户 review C04 现状时强烈反对 `<surface>/{pages,states}/<page-id>.{empty,loading,error,forbidden}.html` 的"工程化"目录，明确指令"极度简化，不要搞状态什么"、"就是一列 HTML 像 function/02-course/ai/F4-AI-原型设计 一样"。所有状态（空 / 加载 / 错 / 无权限）从此由 docs/C06-prd/<feature>/ 文字描述承担，不在原型出图。

### 19-1 · docs/C05-prototype 物理结构重构（119 → 51 文件，-57%）
- 删除全部 6 个 surface 下的 `pages/` / `states/` / `assets/` 子目录
- 将 37 个 `pages/P-*.html` 上提至 `<surface>/` 根（平铺）
- 修正所有上提后 HTML 的相对路径：`../../../../B03-design/` → `../../../B03-design/`（auth + discover-china），`../../../../../function/` → `../../../../function/`（course）
- 修正所有上提后 HTML 顶部 `TARGET-PATH` 注释，回填 `<surface>` 段
- 6 个 `<surface>/index.html` 全量重写：
  - auth / discover-china 用 B04 模板：glass-card + 表格（page-id / 名称 / 入口），引用 `../../../B03-design/prototype-style/{tokens,themes,app}.css` + `app.js` + `proto.bootstrap()`
  - course 用 F4 模板：zy-card 网格，引用 `../../../../function/02-course/ai/F4-AI-原型设计/_assets/{styles.css,prototype.js}`

### 19-2 · /prompt 极简平铺规范同步
- [prompt/A-framework/A00-04-文档目录规划.md](../../prompt/A-framework/A00-04-文档目录规划.md) §C05-prototype 目录树：删除 `pages/` / `states/` / `assets/` 三行，改为 `<surface>/index.html + <page-id>.html` 平铺；硬约束块加入"极简平铺"+"单端 feature 退化形态"
- [prompt/C-product/C05-H01-用户输入-原型方向.md](../../prompt/C-product/C05-H01-用户输入-原型方向.md) 整文件重写：§本阶段不做 加入"状态文件由 C05 文字承担"；§3 必出页面表去掉"必盖状态"列改为"按 surface 分组"；§5 数据来源去掉 mock-data.js 改为"HTML 静态片段"；§10 验收加入"无 pages/states/ 子目录、无 *.empty/loading/error/forbidden.html"
- [prompt/C-product/C05-H02-AI澄清-原型提问.md](../../prompt/C-product/C05-H02-AI澄清-原型提问.md) §B 第 6 条去掉 forbidden 态检查；第 7 条去掉 mock-data.js 改为 HTML 静态片段；第 11 条由"状态四件套"改为"极简平铺自检"
- [prompt/C-product/C05-H03-AI输出-HTML原型规范.md](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md) 整文件重写：触发提示词加"极简原则"块；§输出目录改为双端 + 单端两套平铺示例；硬约束 §0.1 新增"极简平铺"、§5 "4 状态必出"整条删除；§states/ 整节删除；§pages/ 改名为 `<surface>/<page-id>.html`；§自检 增"无 pages/states/assets/vendor 子目录"、"无 *.empty/loading/error/forbidden.html"
- 路径深度统一为 3 ups（B04） / 4 ups（F4），不再有"index 用 3 ups，page 用 4 ups"的两套规则

### 19-3 · 一致性自检（已通过）
- `find docs/C05-prototype -type f | wc -l` = 51 ✅
- `grep -r "../../../../B04\|../../../../../function" docs/C05-prototype` = 0 ✅
- `grep -r "pages/\|states/" docs/C05-prototype/*.html docs/C05-prototype/*/*.html docs/C05-prototype/*/*/*.html` = 0 ✅
- `grep -r "C05-prototype.*pages/\|states/\|\.empty\.html\|\.loading\.html\|\.error\.html\|\.forbidden\.html" docs/` = 0 ✅（C01/C02/C03/C05 均无 stale 引用）
...
- [prompt/B-foundation/B03-S03-AI输出-设计系统.md](../../prompt/B-foundation/B03-S03-AI输出-设计系统.md) §9.1 HTML 示例 4 层相对路径
...
- [docs/B03-design/prototype-style/README.md](../B03-design/prototype-style/README.md) §2/§4 重写为相对路径引用示例

### 18-3 · course 角色三→二收敛（覆盖 /system 实际 roles enum=['super_admin','user']）
- 删除 `content_admin` / `tracks_readonly` / scope 越权概念
- [docs/C06-prd/course/admin/01-overview.md, 02-glossary.md, 03-personas.md, 05-user-journeys.md, 07-business-rules.md, 08-roles-permissions.md, 11-roadmap.md](../C06-prd/course/admin/)
- [docs/C06-prd/course/_shared/glossary.md, business-rules.md](../C06-prd/course/_shared/)
- [docs/C06-prd/_glossary.md](../C06-prd/_glossary.md) §B「角色」整段重写

### 18-4 · C05-prototype surface 极简化（用户："不是说都引用 B04 然后这里就只有 HTML 文件么"）
...
- **现状**：`docs/C05-prototype/` 仅余 119 个 HTML + `_input/` H01 输入 + course `_assets/`（待处理，见 18-5）
- **prompt 同步收紧**：
  - [prompt/A-framework/A00-04-文档目录规划.md](../../prompt/A-framework/A00-04-文档目录规划.md) 新增"★ 硬约束"块明确 surface 只能含 HTML
  - [prompt/C-product/C05-H03-AI输出-HTML原型规范.md](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md)：
...
### 18-5 · 已知待用户决策项（不阻塞）
- `docs/C05-prototype/course/{app,admin}/` 共 20 个 HTML 引用 `function/02-course/ai/F4-AI-原型设计/_assets/styles.css`——`/function` 删除后会断链。建议在删 `/function` 前先 `cp -R function/02-course/ai/F4-AI-原型设计/_assets/ docs/C05-prototype/course/_assets/` 并 sed 改路径；或保留 `/function/02-course/ai/F4-AI-原型设计/_assets/` 不删。
- F4 例外条款在 prompt（A00-04/C05-H03）仍保留——后续若彻底不依赖 /function，应同步删除此例外。

---

## 2026-05-17 · 批次 17 · Round 10 · /prompt C04 改「拷贝→引用」+ C04 原型乱码修复 + 全量交叉链接修复

> **触发原因**：用户在浏览器打开 `docs/C05-prototype/course/admin/pages/*.html` 看到样式乱码——根因：course 页面 `href="_assets/styles.css"` 但页面在 `pages/` 子目录而资产目录名为 `assets/`（无下划线），且整套 C04 把 B04 拷贝到 `<surface>/vendor/proto-style/` + `course/<surface>/assets/` 是 469~1119 行的样式重复脏数据。用户明令：「不要怕工作量，统一给我按引用的模式改！改 prompt 得复制为引用」+「系统课程的内容，你要与 function/02-course/ai/F4-AI-原型设计完全一致」。

### 17-1 · /prompt C05-H01/H03 改「拷贝→引用」

- [prompt/C-product/C05-H03-AI输出-HTML原型规范.md](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md)
  - 触发提示词【运行时资产】块：把「全量拷贝到 `<surface>/vendor/proto-style/`」改为「**禁止**在 feature 目录下任何位置复制 B04，HTML 必须通过相对路径引用 `docs/B03-design/prototype-style/`」；pages 用 `../../../../B03-design/prototype-style/X`，index 用 `../../../B03-design/prototype-style/X`
...
- [prompt/C-product/C05-H01-用户输入-原型方向.md](../../prompt/C-product/C05-H01-用户输入-原型方向.md)
  - 产物表行：`vendor/proto-style/ 全量拷自 B04` → 「HTML 通过相对路径引用 `docs/B03-design/prototype-style/`（不拷贝）」+ 在「本阶段不做」列写明禁止任何 B04 拷贝
  - 弹窗组件来源、token 漂移规则、输出自检 3 处 vendor 表述改为 B04 引用模型

### 17-2 · docs/C05-prototype 全量重构（拷贝→引用）
...
- **重写 HTML 引用**（auth + discover-china 共 88 个 HTML）：
  - pages/states：`"../vendor/proto-style/X"` → `"../../../../B03-design/prototype-style/X"`
  - surface/index.html：`"vendor/proto-style/X"` → `"../../../B03-design/prototype-style/X"`
...
- **修复旧批次遗留 page-id 误链**：
  ...
  - auth/{app,admin}/index.html footer `../../B03-design/prototype-style/README.md` → `../../../B03-design/prototype-style/README.md`（路径深度修正）
  - auth/admin/pages/P-admin-auth-001.html 内 `../../auth/pages/P-admin-auth-001.html` 自指 → 改为 `P-admin-auth-001.html`
- **新增 feature 入口 + 全局入口**：
  - `docs/C05-prototype/index.html`（全局 3 feature × 2 surface 卡片）
  - `docs/C05-prototype/{auth,course,discover-china}/index.html`（feature 级 app/admin 双卡）
...
- 全量交叉链接扫描：`docs/C05-prototype/**/*.html` 共 142 个文件，所有 `href`/`src` 相对路径全部 resolve 通过（0 broken）
...
- [prompt/C-product/C02-P01-用户输入-角色描述.md](../../prompt/C-product/C02-P01-用户输入-角色描述.md) 重写顶部「登录不是全局事」段：auth 是普通 feature；项目无登录需求时整类不存在
- [prompt/C-product/C02-P02-AI澄清-权限提问.md](../../prompt/C-product/C02-P02-AI澄清-权限提问.md) 数据合规问题「本期需要满足的法规」→「项目需要满足的法规」
- [prompt/C-product/C02-P03-AI输出-权限规范.md](../../prompt/C-product/C02-P03-AI输出-权限规范.md) 04-auth-feature-guideline 整文件改写：标题去 `<surface>-auth`、明确「**作为一个普通 feature**，推荐 id `auth`，不要造 `<surface>-auth`」；输出位置示例统一 `auth/<surface>/`；校验项的单/多端路由分流；整文件改为「仅当项目存在登录交互时输出」
...
- [prompt/C-product/C05-H02-AI澄清-原型提问.md](../../prompt/C-product/C05-H02-AI澄清-原型提问.md) 禁问列表加「是否分优先级」；维度 11 P0/P1 改「所有进表页面」；自检项同步
- [prompt/C-product/C05-H03-AI输出-HTML原型规范.md](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md) 「P0 页面必须出 4 态」→「每个进表页面必须出 4 态」；禁止跳过理由加「优先级低」；changelog 模板去「不分期」括注；自检项同步

### 15-5 · 保留的原则性引用（**不删**）

下列表达虽然出现 P0/P1/P2/本期/二期 字样，但属于「禁止规则」本身，保留以强化原则：

- `prompt/C-product/C05-H01-...md` L78「没有 P0/P1/P2 / 优先级 / 待定 / 二期 这种说法」
- `prompt/B-foundation/B01-A01-...md` L68「永不设置『是否本期上线』『二期』这种列」
- `prompt/C-product/C01-R03-...md` L80「永不出现 P0 / P1 / P2 / 本期 / 二期 / MoSCoW」
- `prompt/C-product/C05-H02-...md` L75 自检「没出现『是否分期 / 本期是否只做一部分 / 二期再做』这类问题」
- `prompt/C-product/C05-H03-...md` L24/L76 禁跳过理由列表
- `prompt/B-foundation/B01-A03-...md` L355 同上
- `prompt/B-foundation/B02-X01-...md` L139「不允许『二期再做』」
- `prompt/B-foundation/B02-X03-...md` L221-229 P1/P2 是体验原则编号（非分期）—— 保留
- `prompt/C-product/C02-P03-...md` 角色「优先级」列（冲突时取最高，技术机制）—— 保留
- `prompt/C-product/C04-N02-...md` L22「首屏优先级」（视觉布局优先级）—— 保留
...
- C01/C04 模板变更：**所有未来生成的 C01-R03 / C05-H01/H02/H03 文件**适用；既有文件不受影响（除非选择批量重生）
...
prompt **明确规定** auth 与 auth 是两个独立 feature(各退化为单 surface),原因是两者鉴权链路完全不共享(cookie 域 / Supabase project / session 各自隔离,见 [`_glossary.md §B/E`](../C06-prd/_glossary.md))。**当前 [docs/C06-prd/auth/app/](../C06-prd/auth/app/) + [docs/C06-prd/auth/admin/](../C06-prd/auth/admin/) 结构完全合规,不修改;prompt 也不需要改。**

### 7-1 · course + discover-china 4 surface × 8 = 32 文件实质化

将 [course/{app,admin}](../C06-prd/course/) + [discover-china/{app,admin}](../C06-prd/discover-china/) 的 `01-overview` / `02-glossary` / `03-personas` / `05-user-journeys` / `09-design-summary` / `10-known-issues` / `11-roadmap` / `12-changelog` 共 32 文件从 Round 2 的 7-19 行骨架 **重写为 30-80 行实质内容**,严格对齐数据源:
...
| 修正项 | 处理 | 数量 |
|--------|------|------|
| `C05-prototype/discover-china/{assets,vendor}` 根级残留(规范要求仅在 `<surface>/`) | `rm -rf` 已删 | 2 |
| C02 / C03 / C04 各 surface 缺 `00-index.md` | 补建(框架式,指向本目录内文件 + 上下游链接) | **16** |
| 全部 24 surface 缺 `99-open-questions.md` | 补建(空模板 + Q-ID 命名约定) | **24** |
| [`C06-prd/_global-index.md`](../C06-prd/_global-index.md) 缺 | 创建:4 feature 状态总表 + 业务域映射 `/function/`) | 1 |
| [`C06-prd/_glossary.md`](../C06-prd/_glossary.md) 缺 | 创建:平台 / 角色 / 标识符 / 5 语 / 路由前缀 / 主要业务术语 6 节,显式引用 `function/02-course/prd/01-04` 来源 | 1 |

### Round 6-1 · C05 单页规格(`06-page-specs/<page-id>.md`)全量展开

| Feature / Surface | page-id 数 | 状态 |
|-------------------|:--:|------|
| [`course/app`](../C06-prd/course/app/06-page-specs/00-index.md) | 8 | ✓ |
| [`course/admin`](../C06-prd/course/admin/06-page-specs/00-index.md) | 9 | ✓ |
| [`discover-china/app`](../C06-prd/discover-china/app/06-page-specs/00-index.md) | 3 | ✓ |
| [`discover-china/admin`](../C06-prd/discover-china/admin/06-page-specs/00-index.md) | 4 | ✓ |
| [`auth/app`](../C06-prd/auth/app/06-page-specs/00-index.md) | 9 | ✓ |
| [`auth/admin`](../C06-prd/auth/admin/06-page-specs/00-index.md) | 4 | ✓ |

每个 page-spec 文件结构(9 段):标识 / 上游来源(C03 / C04 / C02 链接) / 一句话价值 / 主交互摘要(≤5 行) / 关键 R-ID / 数据接口边界(留待编码期回填,不再依赖 D) / 视觉与组件(引 [`B03-design/05-components/`](../B03-design/design-system/05-components/)) / 截图占位 / 变更。
...
| [`system/packages/{ui-kit, shared-schemas, shared-i18n, ...}/`](../../system/packages/) | page-spec §7 视觉组件统一引 B03-design |
...
| **R5-C · D03 校验链路重写** | 6 surface × {01-upstream-chain, 02-module-closure, 03-prd-traceability} | **18 文件** | 基于 Round 3 + Round 4 后的真实依赖图;新增"组件引用是否指向 [05-components/](../B03-design/design-system/05-components/) 而非旧 04-status-components.md"检查项;新增 Round 4 PR D 回链确认段 |
...
> F 层最后一块空骨架收口。批次 9 的 Round 2 只为 [`B03-design/design-system/05-components/`](../B03-design/design-system/05-components/) 创建了 13 个文件骨架,本次按上游 [`grules/G2-视觉与交互风格/04-状态与组件.md`](../../grules/G2-视觉与交互风格/04-状态与组件.md) 逐文件填入实质内容。

### Round 4 · 13 组件文件实质化

| 文件 | 内容范围 | 上游 §  |
| ---- | ---- | ---- |
| [00-index.md](../B03-design/design-system/05-components/00-index.md) | 组件清单 + ui-kit 导出名映射 + 全局规则 | §九 |
| [01-buttons.md](../B03-design/design-system/05-components/01-buttons.md) | 5 变体矩阵 + 尺寸/状态/约束 | §二 |
| [02-forms.md](../B03-design/design-system/05-components/02-forms.md) | 布局/输入框/提示文案/复合控件/校验时机 | §四 |
| [03-tables.md](../B03-design/design-system/05-components/03-tables.md) | 容器/空状态/加载/虚拟滚动/可访问性 | §三 |
| [04-modals.md](../B03-design/design-system/05-components/04-modals.md) | 尺寸/视觉/关闭/动效/a11y | §五 |
| [05-drawers.md](../B03-design/design-system/05-components/05-drawers.md) | 抽屉规则 + 移动端 | §六 |
| [06-toasts-alerts.md](../B03-design/design-system/05-components/06-toasts-alerts.md) | Toast 4 类型 + 危险确认弹窗(含高危二次确认) | §七 / §八 |
| [07-empty-loading.md](../B03-design/design-system/05-components/07-empty-loading.md) | EmptyState / Skeleton / Spinner | §三.1 / §三.2 |
| [08-popovers-tooltips.md](../B03-design/design-system/05-components/08-popovers-tooltips.md) | Tooltip + Popover + a11y | (本项目新增) |
| [09-avatars-badges-tags.md](../B03-design/design-system/05-components/09-avatars-badges-tags.md) | StatusTag 6 语义 + Avatar 4 尺寸 + Badge 3 类型 | §一 |
| [10-tabs-accordion.md](../B03-design/design-system/05-components/10-tabs-accordion.md) | GlassTabs 3 变体 + Accordion | (本项目新增) |
| [11-cards-glass.md](../B03-design/design-system/05-components/11-cards-glass.md) | GlassCard + 4 毛玻璃工具类 + 降级 + 性能 | §九 + 01 §四 |
| [12-decorations.md](../B03-design/design-system/05-components/12-decorations.md) | 分隔线/品牌光晕/底纹/图标/禁忌 | (本项目新增) |
...
> **二次审计发现**:批次 9 修正 `course` / `discover-china` 时,遗漏了 [`prompt/A-framework/A00-04-文档目录规划.md`](../../prompt/A-framework/A00-04-文档目录规划.md) §四.5 第 280–289 行示例明确指出的 **`<surface>-auth` feature 也须有退化 `<surface>/` 子层**(例如 `C05-prototype/auth/admin/...`),当前 `auth` / `auth` 的内容直接挂在 feature 根,与 `course/{app,admin}/` 形态不对称,违反"多端项目所有 feature 统一形态"原则。
...
| 阶段 | feature 根原结构 | 下沉后 |
| --- | --- | --- |
| C03-ia | `auth/{00..07-*,_input,99-open}` | `auth/app/{00..07-*}` + 根 `_input,99-open` |
| C04-pages | `auth/P-auth-00X.md` | `auth/app/P-auth-00X.md` |
| C05-prototype | `auth/{index.html,pages,states,vendor,assets,feature.css/js,mock-data.js,README,changelog}` | `auth/app/<同结构>` |
| C06-prd | `auth/{00..12-*,06-page-specs}` | `auth/app/<同结构>` |
...
| 范围 | 操作 | 数量 |
| ---- | ---- | ---- |
| `C03-ia/{course,discover-china}/` | 新建 `_shared/`,迁入 `state-machines.md`(原 03-)与 `flows-shared.md`(02-flows 副本);新建 `{app,admin}/` 骨架 | 4 _shared 文件 + 4 surface 目录 |
| `C04-pages/{course,discover-china}/` | 新建 `{app,admin}/`,按 `P-app-*` / `P-admin-*` 命名分发 | 24 文件迁移(course 8+9,discover-china 3+4) |
| `C05-prototype/course/` | 新建 `{app,admin}/{vendor/proto-style,pages,states,assets}/`,从 `function/02-course/ai/F4-AI-原型设计/` 拷贝 17 HTML(P-A-* → admin,P-C-* → app),vendor 从 [`docs/B03-design/prototype-style/`](../B03-design/prototype-style/) 拷,补 `index.html` / `feature.css` / `feature.js` / `mock-data.js` / `README.md` / `changelog.md` | 每 surface 一套 |
| `C05-prototype/discover-china/` | 把原平铺 `pages/` / `states/` / `vendor/` / `index.html` 按 surface 拆到 `{app,admin}/` 各自独立目录 | 7 HTML 拆 + 25 states 拆 + 2 套 vendor |
| `C06-prd/{course,discover-china}/` | 新建 `_shared/` 与 `{app,admin}/06-page-specs/` 骨架 | 12 目录 |
...
| 范围 | 操作 | 数量 |
| ---- | ---- | ---- |
| `C03-ia/{course,discover-china}/{01-feature-catalog,02-flows,04-pages,05-navigation,06-coverage-matrix}.md` | 拷到 `{app,admin}/` 各一份(初版相同,Round 3+ 按端过滤),`TARGET-PATH` 更新,根级删除,加 surface banner | 20 文件 |
...
| `C06-prd/{course,discover-china}/PRD.md` | **大爆炸**:按第 1..12 章拆成 12 文件 × 2 surface(00-index / 01-overview / 02-glossary / 03-personas / 04-feature-catalog / 05-user-journeys / 06-page-specs/00-index / 07-business-rules / 08-roles-permissions / 09-design-summary / 10-known-issues / 11-roadmap / 12-changelog),`_shared/` 同步 `glossary.md`(第 2 章)与 `business-rules.md`(第 7 章),根 PRD.md 删除 | 60 surface 文件 + 4 _shared 文件 |
| `B03-design/design-system/` | 文件重命名以对齐 [`prompt/A-framework/A00-04-文档目录规划.md §四`](../../prompt/A-framework/A00-04-文档目录规划.md):`04-status-components.md` → `04-status-colors.md`、`05-interactions.md` → `06-interactions.md`、`06-responsive-dark.md` → `07-responsive-dark.md`、`07-icons-imagery.md` → `99-extension-icons-imagery.md`(标记为非规范扩展) | 4 文件重命名 + TARGET-PATH 修 |
| `B03-design/design-system/05-components/` | 新建 13 文件骨架(00-index + 01-buttons / 02-forms / 03-tables / 04-modals / 05-drawers / 06-toasts-alerts / 07-empty-loading / 08-popovers-tooltips / 09-avatars-badges-tags / 10-tabs-accordion / 11-cards-glass / 12-decorations) | 13 文件 |

### 影响与回链
- **F 层(B03-design 重命名)触发**:已开工 feature(`auth` / `auth` / `course` / `discover-china`)的 D03 V 链路需在 Round 3 重跑校验交叉引用是否仍命中(组件引用现需指向 `05-components/<file>` 而非旧 `04-status-components.md`)。
- **多 surface 修正不破坏 D01**(数据模型按 feature 共享,本身就是 single,无需拆 surface)。
- **C05-prototype** 每 surface 独立 `vendor/proto-style/` 已就位,但 `index.html` / `feature.css` / `feature.js` / `mock-data.js` 当前是占位/原版拷贝,Round 4 将按各 surface 真实 page-id 重写。

### 后续 Round 3+ 待办(已记入待办列表)
1. C03-ia / C06-prd / D02-api 各 surface 文件按端过滤实质内容(剔除对端独有的 page / endpoint / 业务规则);
2. C06-prd 各 surface 的 `06-page-specs/` 按 page-id 展开单页 spec 文件;
...
5. C05-prototype 各 surface 的 `feature.css` / `feature.js` / `mock-data.js` 按 OP-ID 实质化;
...
- 关键决策:
  - schema 统一 `zhiyu`(表名前缀 `course_*`),不另建 `zhiyu_course`(已同步修正 [C05 PRD §13](../C06-prd/course/PRD.md));
...
### 同步修订
- [`docs/C06-prd/course/PRD.md` §13](../C06-prd/course/PRD.md):schema 名从 `zhiyu_course` 改回 `zhiyu`(course_* 前缀);表数 18 → 15;OP 数细化为 19+24+3;错误码 ~40 → 55。
...
### C04 原型契约(2 文件)
- [`docs/C05-prototype/course/README.md`](../C05-prototype/course/README.md):17 HTML 原型 → 17 P-ID 映射 + 12 组件清单(`<KpCard>` 7 变体 / `<QuestionRenderer>` 12 种 / `<CountdownTimer>` / `<ProgressMap>` 等)+ 6 状态契约 + Design Token 契约(`--track-color-*` / `--lesson-*` / `--answer-*`)+ 多语对齐(`course.*` key 前缀)
- 原则:不复制 HTML 资产,实现端按本契约 + F3 + B03 token 三方对齐还原;原型与契约冲突时以 F3 + 契约为准

### C05 PRD(3 文件)
- [`docs/C06-prd/course/PRD.md`](../C06-prd/course/PRD.md):§7 业务规则 **40 条 BR-ID**,分 7 段:BR-STRUCTURE(S01..S07 骨架 / 命名)/ BR-CONTENT(C01..C10 生命周期 / 5 语完整性 / 版本治理 / LWW)/ BR-LEARNING(L01..L07 节内 / 答题副作用 / 离线队列 / 多端同步)/ BR-SRS(R01..R04 Leitner 间隔 / 题型轮换)/ BR-EXAM(E01..E08 4 类考试 / 动态均分 / 阶段考解锁 / 不可重考 / 试抽不写 attempt)/ BR-REPORT(RP01..RP04 举报聚合 / 历史不追溯)/ BR-AUTH-PERM(P01..P06 多语 / 订阅控制 / 行级 tracks_scope)/ BR-SYS(Y01..Y04 系统副作用)
...
### 已知保留项
- `AUTH_USE_USER_ENTRY`:继续按 [`C02-permissions/99-open-questions.md Q-2026-05-16-01`](../C02-permissions/99-open-questions.md) 留待 F 层修订;不阻断批次 7 / 8;
...
- [`docs/C01-requirements/discover-china/baseline.md`](../C01-requirements/discover-china/baseline.md) §4:补 5 语 locale key 集合(`zh/en/vi/th/id`)与 [`C02-permissions §4`](../C02-permissions/03-authz-mechanism.md) 对齐;新增文章编码与 seq_no 格式正式规约 → D01 schema 输入完备;
- [`docs/C03-ia/{auth,auth,discover-china}/06-coverage-matrix.md`](../C03-ia/):在文件头添加 "ID 缩写约定" 说明,明确短形与完整形的映射,避免 D03 V01 跨文件追溯歧义;
- 维持决策:`AUTH_USE_USER_ENTRY` 仍按 [`C02-permissions/99-open-questions.md Q-2026-05-16-01`](../C02-permissions/99-open-questions.md) 留待下次 F 层修订统一补登,不阻断批次 6。
...
### C04 HTML 原型(共 43 文件)
- vendor/proto-style/ 4 文件由 B03-design/prototype-style/ v1.0 拷贝;
...
### C05 PRD
- [`docs/C06-prd/discover-china/PRD.md`](../C06-prd/discover-china/PRD.md):13 章完整 PRD,12 条 BR 业务规则,角色权限矩阵,路线图 v1.0..v2.0。
...
> 修正批次 3/4 当时将 C04 标注为"暂缓占位"违反框架"一次性出齐"硬约束;本次按 [`prompt/C-product/C05-H03-AI输出-HTML原型规范.md`](../../prompt/C-product/C05-H03-AI输出-HTML原型规范.md) 补齐全部原型产物。

### B04 实化(v1.0)
- [`docs/B03-design/prototype-style/tokens.css`](../B03-design/prototype-style/tokens.css):全套 CSS 变量(颜色/字体/间距/圆角/阴影/毛玻璃/动效),亮模式默认 + 暗模式覆盖在 themes.css;
- [`docs/B03-design/prototype-style/themes.css`](../B03-design/prototype-style/themes.css):`[data-theme="dark"]` 完整覆盖 + `prefers-reduced-motion` 兜底;
- [`docs/B03-design/prototype-style/app.css`](../B03-design/prototype-style/app.css):reset + 远景红光晕 + `.glass-*` / `.btn-*` / `.input` / `.toast` / `.skeleton` / `.proto-switcher` / `.env-badge`,响应式 768/375 双断点;
- [`docs/B03-design/prototype-style/app.js`](../B03-design/prototype-style/app.js):`window.proto.{bootstrap,setTheme,toggleTheme,toast,cooldown,devtools.mountStateSwitcher,on}` 全局 API,FOUC 内联主题恢复;
- 状态从「占位」→「v1.0 已填充」。
...
- [入口](../C05-prototype/auth/app/index.html)。

### C04 `auth` 全套(28 文件)
- 4 page-id × (1 default + 3 必出状态) + 1 权限页 forbidden 态 = pages/ 4 + states/ 13;
- 其余资产同上;
- [入口](../C05-prototype/auth/admin/index.html)。
...
| C02 I | `docs/C03-ia/auth/admin/*` (00..07 + 99 + _input) | 10 | 已冻结 |
| C03 N | `docs/C04-pages/auth/admin/P-001..004 + 99 + _input` | 6 | 已冻结 |
| C04 H | `docs/C05-prototype/auth/admin/{README,_input}` | 2 | **占位** (与 B04 prototype-style 同步 deferred) |
| C05 E | `docs/C06-prd/auth/admin/*` (00..12 + 99 + 06/00-index + _input) | 15 | 已冻结 |
...
**V01 轻度警告**:`AUTH_USE_USER_ENTRY` 错误码尚未列入 [`B02-03 §4`](../C02-permissions/03-authz-mechanism.md);已记入 [`C02-permissions/99-open-questions.md`](../C02-permissions/99-open-questions.md),下一轮 F 层修订时补入;不阻断本 feature 冻结。
...
| C02 I | `docs/C03-ia/auth/app/*` (00..07 + 99 + _input) | 10 | 已冻结 |
| C03 N | `docs/C04-pages/auth/app/P-auth-001..009 + P-001.scenarios + 99 + _input` | 12 | 已冻结 |
| C04 H | `docs/C05-prototype/auth/app/{README, _input}` | 2 | **占位** (与 B03-design/prototype-style 一致 deferred) |
| C05 E | `docs/C06-prd/auth/app/*` (00..12 + 99 + 06-page-specs/00-index + _input) | 17 | 已冻结 |
...
| B02 · P | `docs/C02-permissions/*` | 已冻结 | 由 `grules/G3-权限与认证规范/` 迁入，剥离 `05-注册流程.md` → 留给 `<surface>-auth` feature |
| B03 · X | `docs/B02-ux/*` | 已冻结 | 反推：基于"东南亚汉语学习 + 毛玻璃 + 教育 + 游戏化"四组关键词整理 |
| B04 · S | `docs/B03-design/design-system/*` | 已冻结 | 由 `grules/G2-视觉与交互风格/` 迁入；`prototype-style/` 资产留待 C04 首次原型时补齐 |
...
| `course` | C04 H | 已冻结 · 含原型 | 由 `function/02-course/ai/F4-AI-原型设计/` 迁入 `docs/C05-prototype/course/<surface>/` |
...
- **B03 设计系统**：补齐 `docs/B03-design/design-system/` 全套 10 文件（`_input/visual-input.md` / `00-index` / `01-tokens` / `02-layout` / `03-navigation` / `04-status-components` / `05-interactions` / `06-responsive-dark` / `07-icons-imagery` / `99-open-questions`）；与 `grules/G2-视觉与交互风格/` 内容一致并新增 `07-icons-imagery.md`（grules 缺失）。
- **B03 原型样式骨架**：建立 `docs/B03-design/prototype-style/` 占位（`README.md` / `tokens.css` / `themes.css` / `app.css` / `app.js`），约定待 C04 首次产出 HTML 原型时填充；与 `system/packages/ui-kit/src/tokens/` 是同一份内容的两种载体。
- **B01-09-auth-infra 与 B02 对齐**：将残留的多角色 `roles[]` / `requireRole('admin_*')` / 前端登出清 `localStorage` 等措辞，统一改为单数 `role` / `requireRole('super_admin')` / 调 `POST /v1/auth/logout` 清 HttpOnly Cookie；增加交叉引用到 [`C02-permissions/02-auth-flow.md`](../C02-permissions/02-auth-flow.md)。
