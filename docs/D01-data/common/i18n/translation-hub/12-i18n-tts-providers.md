...
| 项 | 值 |
|----|-----|
| 关联 R-ID | R-i18n-065, R-i18n-066 |
| 业务定义 | TTS 配音供应商配置（支持多供应商） |
| 状态机 | 无 |
...
| provider_name | text | 是 | 无 | 否 | 供应商名称 | 如"Azure TTS"，长度≤100 |
| api_endpoint | text | 是 | 无 | 否 | API 端点 | 合法 URL |
| api_key_encrypted | text | 是 | 无 | 否 | 加密的 API Key | AES-256 加密存储 |
| request_template | jsonb | 否 | null | 否 | 请求参数模板 | 全局默认参数 |
...
> - api_key_encrypted 在 API 返回时脱敏。
> - 不硬编码任何 TTS 供应商平台，完全自定义。

## 关系

| 目标表 | 基数 | 外键字段 | 删除策略 |
|--------|------|----------|----------|
| i18n_tts_models | 1:N | provider_id | CASCADE |
