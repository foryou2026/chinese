...
| 项目 | 决定 |
|------|------|
| Paddle Webhook 路径 | `POST /api/v1/app/webhooks/paddle` |
| 签名验证 | 强制使用 Paddle 官方 SDK `verifyWebhookSignature`，拒绝任何未通过验签的请求 |
| 幂等性设计 | `transaction_id` 加唯一索引；处理前先在事务中检查订单状态，防止网络重发导致重复发货/充值 |
| IP 白名单 | 可选：限制仅 Paddle 服务器 IP 段可访问 |

> Paddle 可能因网络波动重发同一支付通知。数据库层必须设计防重入逻辑，确保付一次钱绝不重复处理。

## 第三方 AI 接口调用

| 项目 | 决定 |
|------|------|
| 火山引擎语音模型 | 统一 HTTP 模式调用，不使用 WebSocket |
| 理由 | 减小服务器长连接压力，架构更简单 |
| 耗时处理 | 语音类耗时请求走 BullMQ 异步队列，前端轮询或 SSE 获取结果 |

## 长任务处理

| 场景 | 策略 |
|------|------|
| 耗时操作（语音生成、报表等） | 返回 `202 Accepted` + 任务 ID |
