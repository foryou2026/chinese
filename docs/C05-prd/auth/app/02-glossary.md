...
| **Auth Hook (before_user_created)** | GoTrue 注册前置 webhook，落库 `zhiyu.profiles` + 校验邀请码（v2）|
| **User Session 记录** | `zhiyu.user_sessions` 行，包含 session_id / refresh_jti / device_id / device_name / 最后活跃时间 |
