...
> **阶段**：V 开发前校验
> **谁产出**：AI（项目分解师）
> **落盘**：`docs/B0-validation/<feature-id>/02-stories.md`（或 global）
...
```
V1 一致性校验已通过。请你扮演"项目分解师"，按 /prompt/40-V-AI输出-Story拆分.md
将本期所有 R-ID 拆为 Story。
- 单 Story 工作量目标：≤ 半天（一次提交可完成）
- 每 Story 必须有：唯一 ID、业务价值、对应 R-ID、对应文档清单、DoD、验收用例 SC-ID
- 输出 mermaid gantt 与依赖图
- 落盘 docs/B0-validation/<feature-id>/02-stories.md
```
...
```markdown
<!-- TARGET-PATH: docs/B0-validation/<feature-id>/02-stories.md -->
...
|----------|------|------|---------|--------------|------|-------|------|
| S-001 | 数据库表创建 + 迁移 | infra | R-001~005 | docs/40-data/course/02-entities/* | 2h | P0 | — |
