...
| BR-S07 | KP / Lesson / Chapter / Stage / Track 顺序由 句子顺序号 管理,管理端可拖拽重排;持久化后唯一 | R-014 |
...
| BR-C02 | 导入 KP / Question 默认 发布态=`false`;批量 / 行内发布显式触发 | R-022 |
...
| BR-C07 | 删除 KP / Question:若被 节-知识点关联 引用或 `考试记录.snapshot` 引用,**禁止硬删**,只能下架 | R-017, R-025 |
| BR-C08 | 媒资按 `hash` 全局去重;有 `ref_kp_id / ref_q_id` 引用时禁止软删 | R-019 |
| BR-C09 | 媒资软删 30 天后由 cron 物理清理 | R-019 |
| BR-C10 | LWW 后写覆盖:保存请求带 loaded_更新时间,服务端比对若不一致则覆盖并返被覆盖管理员名,前端 Toast 提示 | (横切)|
...
| BR-L02 | 节末小测可跳过(`is_quiz_required=false`);跳过则 学习进度.status=`in_progress`,仍解锁下一节 | R-006 |
...
| BR-L05 | 答题副作用:正确 → 复习队列.box+1, due_at += 间隔表`;错误 → `box=1, due_at=`明天` | R-026 |
| BR-L06 | 弱网答题写本地队列(`localforage`),回网后批量同步 `POST /app/answer:batch` | FX-course-01 |
| BR-L07 | 多端进度以服务端 学习进度 / 复习队列 / 考试记录 为准,前端只缓存 | R-030 |
...
| BR-E03 | `POST /exam/{id}/start` 写 考试记录,按 blueprint 抽题写 snapshot;`POST /exam/{attempt}/submit` 批量判分 | R-011 |
...
| BR-E07 | 试抽预览仅渲染,**不写 考试记录**,不算 attempt | R-020 |
...
| BR-RP03 | 采纳 / 忽略不自动改 questions.发布态;改与不改由 admin 显式操作 | R-018 |
| BR-RP04 | 修正题目后 `version+1`;答题记录 历史不动,新作答取新 version | R-024 |
...
| BR-P04 | 用户端 API + 视图层强制 发布态=`true` 且父级全发布;违反返 404 / 410 | R-029 |
| BR-P05 | admin 端中间件统一校验 `role === 'admin'`；非该角色访问 `/api/admin/course/*` 返 403 | R-013..021 |

### 7.8 BR-SYS(系统副作用)

| BR-ID | 规则 | 关联 R |
|-------|------|--------|
| BR-Y01 | 答题记录 写入触发 复习队列 升降盒 + 重算 `due_at`(BR-L05) | R-026 |
| BR-Y02 | 阶段考通过 → 自动写下一阶段全部节 学习进度.status='unlocked'(BR-E05) | R-027 |
