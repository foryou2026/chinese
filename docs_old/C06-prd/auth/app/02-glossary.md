...
| **Refresh Token (zhiyu-rt)** | 鉴权后端颁发的 refresh token，30d 滚动更新，HttpOnly cookie |
| **CSRF Token (zhiyu-csrf)** | 32 字节随机串，**非 HttpOnly**，双提交机制 |
| **Cookie 存储适配 Adapter** | 自实现的 鉴权 SDK `鉴权 SDK 客户端配置.auth.storage` 适配器，通过自托管 API（`/api/auth/cookie/*`）读写 cookie |
| **Auth Hook (before_user_created)** | 鉴权后端注册前置 webhook，落库 用户档案 + 校验邀请码（暂不支持）|
...
| **Recovery Session** | 通过密码重置链接换取的临时会话，只允许调用 更新用户资料，不允许其他业务接口 |
