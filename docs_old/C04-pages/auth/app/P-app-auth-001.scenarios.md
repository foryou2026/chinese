<!-- TARGET-PATH: docs/C04-pages/auth/app/P-app-auth-001.scenarios.md -->
...
| S2 | 邮密登录带 redirect | `?redirect=/me/profile` | 登录后跳 对应页面 |
...
| S8 | Google 一键 | 点 Google | 跳 Google → 对应页面 → 跳 `/` |
| S9 | Google 取消 | Google 同意页点取消 | 静默回 对应页面，不弹 Toast |
| S10 | `?reason=kicked` 直接访问 | URL `?reason=kicked` | 顶部 Toast「您的账号在其他设备登录…」+ idle |
| S11 | 已登录访问本页 | 有效 cookie | 自动跳 `/` |
| S12 | 5xx 模拟 | mock 登录接口返 500 | Toast「服务异常」+ 字段值保留 |
| S13 | 键盘提交 | Tab 到密码 + Enter | 触发提交（form `onSubmit`） |
| S14 | 主题切换不丢值 | 切换 dark/light | 字段值保留 |
| S15 | 第 4 设备登录 | D4 同账号登录 | 200 + 跳 `/`；早设备下次 refresh 后跳 对应页面 |
