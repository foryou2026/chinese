...
1. **本阶段同时承担"路由"与"接口"两件事**：
   - 路由：把 C03 IA `04-pages.md` 中的 page-id 反向映射成 URL 路径（含鉴权、URL 命名规则、语言策略）
   - 接口：API endpoint
2. **page-id 必须 100% 来自 C03 04-pages.md**；不得新增/删改 page-id（如发现需要新增页面，必须先回 C03 修订）。
3. **API 必须覆盖 C03 03-state-machines.md 的所有状态转移**；每条 SM 转移都要有承接接口。
...
多 surface feature 请按形态 B 拆子目录（`<surface>/...`）且后端路由以 `/api/<surface>/` 起始。
...
### 形态 B：多 surface（同一 feature 多端拆）

> 遵守 A00-04 §四.5：多端 feature 的路由与接口按 `<feature>/<surface>/` 拆子目录；后端路由前缀遵守 B01 08-surfaces §2：必须以 `/api/<surface>/` 起始。

```
docs/D02-api/_global-routes.md
docs/D02-api/<feature-id>/
  00-index.md                       # 本 feature 跨 surface 的总入口（实体映射、SM 映射可跨 surface 共享）
  04-error-codes.md                 # 错误码可跨 surface 共用
  05-concurrency.md
  06-events.md
  99-open-questions.md
  <surface>/                        # 例：admin/、app/、site/
    01-routes-delta.md              # 本 surface 路由，全部以 /api/<surface>/ 起始
    02-overview.md
    03-endpoints/
      <method>-<path>.md
```
...
|----|------|------|------|------|------|----------------|-------------|------|
| API-course-create-courses | POST | /api/courses | 创建课程 | ROLE-EDITOR | R-002 | OP-2@P-040 | SM-02:T-1 | 03-endpoints/post-courses.md |
| API-course-list-courses | GET | /api/courses | 课程列表 | * | R-001 | OP-1@P-002 | — | 03-endpoints/get-courses.md |

> **API-ID 命名规则**（A00-03 §四）：`API-<feature>-<verb>-<noun>`；verb 取 create/list/get/update/delete/<业务动作名>；noun 取资源复数名。
...
## 3. page-id ↔ URL 映射（必须与 C03 04-pages.md 一一对应）
...
## 5. 自检
- [ ] C03 04-pages.md 所有 page-id 全部出现且仅出现一次
- [ ] 没有臆造 C03 没有的 page-id
- [ ] 鉴权与 C03 05-navigation.md 角色可见性一致
- [ ] 兜底页齐
```
...
- **API-ID**：API-<feature>-<verb>-<noun>（例：API-course-create-courses）
...
## 状态转移（如承接 SM）
| SM-ID:T-X | 起态 | 终态 | 触发条件（与 C03 SM 表完全一致）| 后置动作 |
|----------|------|------|----------------------------|---------|
...
|------|------|------|-------------|--------------|---------|
| 40901 | 409 | 课程上架后名称不可改 | … | … | API-course-update-courses |
```
